...
u8 usmart_sys_cmd_exe(u8 *str)
{
	...
	{
		if(strcmp(sfname,sys_cmd_tab[i])==0)break;
	}
	switch(i)
	{					   
		case 0://查询指令
...
{/*
	u8 sta,len;  
		...
		//printf("Command\r\n");
		//UsmartRxBufferFS[Usmartlen]='\0';	//在末尾加入结束符.
		//sta=usmart_dev.cmd_rec(UsmartRxBufferFS);//得到函数各个信息
		if(sta==0)usmart_dev.exe();	//执行函数
		else 
		{  
			//len=usmart_sys_cmd_exe(UsmartRxBufferFS);
...
void usmart_exe_usb(uint8_t *pata,uint8_t len)
{
	u8 sta;
	pata[len]='\0';	//在末尾加入结束符.
	sta=usmart_dev.cmd_rec(pata);//得到函数各个信息
	if(sta==0)
		{
		usmart_dev.exe();	//执行函数
		}
